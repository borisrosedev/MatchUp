...
export default class LogContainer extends ContainerModel {
    ...
    constructor(onNavigate: OnNavigateType, isLoggingIn: boolean){
        ...
        if(!this.isLoggingIn){
            ...
            this.confirmedPasswordErrorSection = document.getElementById("confirmed-password-error")
        }
    ...
    async onLoginSubmit(emailValue: string, passwordValue: string){
        ...
        const passwordFormResult = this.formatService.checkPasswordFormat(passwordValue)
    ...
    onRegisterSubmit(emailValue: string, passwordValue: string, confirmedPasswordValue: string){
